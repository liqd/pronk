...
run cfg@Config{..} = do
  ...
  let !interval | requestsPerSecond == 0 = 0
                | otherwise = realToFrac (fromIntegral concurrency /
                                          requestsPerSecond)
  ch <- newChan
  forM_ (zip reqs [0..]) $ \(numReqs, stateSelector) -> forkIO . withManager $ \mgr -> do
    let cfg' = cfg { numRequests = numReqs }
    liftIO . writeChan ch =<< try (client cfg' stateSelector mgr interval)
...
client Config{..} stateSelector mgr interval = loop initialRequestGenerator 0 []
  where
    ...
    initialRequestGenerator = case request of
            RequestGeneratorConstant r
                -> RequestGeneratorConstant r
            RequestGeneratorStateMachine name states trans
                -> RequestGeneratorStateMachine name [concat (repeat states) !! stateSelector] trans
